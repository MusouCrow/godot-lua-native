...
- 本文档仅约束 `src/**` 的实现方式与对外行为（模块组织、句柄/所有权、线程/错误模型等）。
- `src/**` 下的 C++ 代码规范：必须遵守 `src/cpp_guidelines.md` 的约定。
- 构建、产物落点、如何运行 headless 测试：见仓库根 `README.md`。
...
- Lua 侧以模块承载导出函数：建议统一使用扁平命名 `native_<domain>`。
...
- **不掩盖错误**：遇到失败必须输出可读的错误信息，并用返回码/退出码明确体现失败。
- **错误必须可读**：错误信息包含：
  - 模块/函数名
...
- **参数错误/状态错误（本插件可判定）**：打印可读错误信息，并返回非 0（或 `-1`）错误码。
- **Godot 调用失败/不支持**：打印可读错误信息，并返回非 0（或 `-1`）错误码。

> 说明：Godot 内部很多失败路径是 `ERR_FAIL_*` 打日志并返回默认值，无法“自动捕获为异常”。因此需要在每个 API 封装层显式检查前置条件、返回码或可观察状态，并把失败转成“可读错误信息 + 明确返回码”。

### 约定：宿主执行接口

- 对外提供的“执行 Lua 文件/字符串”的宿主接口（例如 `run_file/run_string`）以 **返回值作为退出码**：
  - `0`：成功
  - 非 `0`（或 `-1`）：失败
- 失败时同时通过 Godot 的 `printerr`（或等价接口）输出可读错误信息，便于在命令行/日志中定位问题。
...
- **只在宿主入口做主线程检查**：例如 `LuaHost.run_file/run_string` 等“执行 Lua”的入口必须校验主线程。
- `native_*` 模块内的导出函数默认**不重复**做主线程检查，避免每个函数带来样板与运行时开销。
- 约束：任何新增“执行 Lua”的入口都必须复用同一套主线程检查策略；否则模块内不检查会导致非主线程调用 Godot API 的风险。
