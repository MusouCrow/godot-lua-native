...
> 原则：先保证正确性与清晰度；只有在明确观测到瓶颈后再优化，避免过早复杂化。

- 绑定层保持薄：导出函数主要做 Lua 栈交互/校验/返回值组织；复杂流程下沉到可复用的 C++ 层。
- 减少正常路径分配：字符串拼接、错误消息格式化尽量放在失败路径；正常路径避免临时 `String`/UTF-8 转换的连锁分配。
- 避免高频日志：`printerr/print` 只用于错误或明确调试场景，避免在未来的高频接口（如每帧调用）里输出。
- 避免重复初始化/注册：模块注册、Lua state 初始化只做一次；不要在高频路径里重复 `requiref`/重复开库等。
- 以可测量为前提：优化前先用最小计数/日志或 Godot profiler 定位，再做针对性改动。
